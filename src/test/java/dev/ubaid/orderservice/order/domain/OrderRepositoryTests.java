...
public class OrderRepositoryTests {
    ...
    void whenCreateOrderNotAuthenticatedThenNoAuditMetaData() {
        Order rejectedOrder = OrderService.buildRejectedOrder("1234567890", 1);
        StepVerifier
                .create(orderRepository.save(rejectedOrder))
    ...
    void whenCreatedOrderAuthenticatedThenAuditMetaData() {
        Order rejectedOrder = OrderService.buildRejectedOrder("1234567890", 2);
        StepVerifier
                .create(orderRepository.save(rejectedOrder))
                .expectNextMatches(order -> order.createdBy().equals("ubaid") && order.lastModifiedBy().equals("ubaid"))
                .verifyComplete();
    }
}
